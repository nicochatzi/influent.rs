use crate::client::{Client, ClientError, Credentials, Precision};
...
mod tests {
    ...
    async fn test_write_one() {
        ...
        if let Err(e) = client
            .write_one(Measurement::new("key"), Some(Precision::Nanoseconds))
            .await
        {
    ...
    async fn test_write_many() {
        ...
        client.add_host("http://localhost:8086");
        assert!(client
            .write_many(&[Measurement::new("key")], Some(Precision::Nanoseconds))
            .await
            .is_ok());
    }
}
